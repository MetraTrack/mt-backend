...
- Let `HttpService` throw on non-2xx responses. Catch at the calling service level if domain-specific handling is needed.
- Log the failure with context: url, method, and error message.
- Map external errors to NestJS exceptions (`BadGatewayException`, `ServiceUnavailableException`) at the boundary.
...
## Bot Callback Pattern

The bot callback (`BotCallbackService`) follows a fire-and-forget pattern: call it with `void` and never await in the main response path. Failures are logged, never surfaced to the caller.
